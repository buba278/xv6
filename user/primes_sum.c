...
// thought about trying to levarage straight from primes ie calling it then grabbing stdout
// but i dont know how to do that so just edited primes
...
int main(int argc, char *argv[]) 
{
    ...
    if (fork() == 0) {
        ...
        while (1) {
            ...
            // PRIMESSUM ADDITION
            int divisor;
            int prev_divsor = divisor;
            if (read(fd[0], &divisor, sizeof(divisor)) <= 0) break;

            int prime_sum = divisor + prev_divsor;
            if (divisor != 2) {
                fprintf(2, "%d\n", prime_sum);
            }
